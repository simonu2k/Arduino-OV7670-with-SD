...
void Init_VGA(){
  ...
  WriteOV7670(0x3E, 0x00);//COM14
  WriteOV7670(0x72, 0x11);//
  WriteOV7670(0x73, 0xF0);//
  WriteOV7670(0x17, 0x16);//HSTART
  WriteOV7670(0x18, 0x04);//HSTOP
  WriteOV7670(0x32, 0xA4);//HREF
  WriteOV7670(0x19, 0x02);//VSTART
...
void Save_Image(String title){
  ...
  while (h--){
    ...
    while (w--){
      while ((PIND & 4));   //wait for low
        dataBuffer[319-byte_count] = (PINC & 15) | (PIND & 240);
      while (!(PIND & 4));  //wait for high
      while ((PIND & 4));   //wait for low
      while (!(PIND & 4));  //wait for high
...
void loop(){
  int index = 0;
  Serial.println(String("Capturing Started"));
  delay(500);
  while(1){
    noInterrupts(); //Disable all interrupts
    //QVGA_Image(String(index) +String(".bmp"));
    Save_Image(String(index) +String(".bmp"));
    interrupts();
    Serial.println(String(index) + String(" Finished"));
    index++;
    delay(500);
  }
}
